...
@Component(AreaMinReplyResponder.KEY)
public class AreaMinReplyResponder implements ReplyResponder {
    ...
    public AreaMinReplyResponder(UserRepo users, SentMessageRepo sentMessages, ConfigInfoProvider configInfoProvider, Validator validator) {
        this.users = users;
    ...
    public void handle(Message message, TelegramBot bot) {
        ...
        if (!matcher.matches()) {
            var sendMessage = new SendMessage(message.chat().id(), "❌ Wrong input! Please enter a number without any separators like ',' or '.'");
        ...
        if (!results.isEmpty()) {
            ...
            var errorDescription = String.format("❌ Min area m² %s", errorMessage);
