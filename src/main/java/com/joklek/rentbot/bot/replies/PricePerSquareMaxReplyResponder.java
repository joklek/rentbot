...
@Component(PricePerSquareMaxReplyResponder.KEY)
public class PricePerSquareMaxReplyResponder implements ReplyResponder {
    ...
    public PricePerSquareMaxReplyResponder(UserRepo users, SentMessageRepo sentMessages, ConfigInfoProvider configInfoProvider, Validator validator) {
        this.users = users;
    ...
    public void handle(Message message, TelegramBot bot) {
        ...
        var pricePerSquareMax = match.equalsIgnoreCase("any") ? null : Integer.parseInt(match);
        ...
        if (!results.isEmpty()) {
            ...
            var errorDescription = String.format("❌ Price per m² max %s", errorMessage);
